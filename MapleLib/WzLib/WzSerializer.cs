    ...
    public abstract class WzXmlSerializer : ProgressingWzSerializer
    {
        ...
        protected void WritePropertyToXML(TextWriter tw, string depth, WzImageProperty prop, string exportFilePath)
        {
            ...
            else if (prop is WzDoubleProperty)
            {
                ...
                string str3 = Convert.ToString(property5.Value, formattingInfo);
                if (!str3.Contains(","))
                    str3 = str3 + ",0";
                tw.Write(string.Concat(new object[] { depth, "<double name=\"", XmlUtil.SanitizeText(property5.Name), "\" value=\"", str3, "\"/>" }) + lineBreak);
            }
