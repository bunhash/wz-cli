...
namespace MapleLib.WzLib.WzProperties
{
    ...
    public class WzPngProperty : WzImageProperty
    {
        ...
        public (int, int) FormatBytes { get { return (format, format2); } set { (format, format2) = value; } }
        public ushort Header { get { return header; } set { header = value; } }
        ...
        public void SetCompressedBytes(byte[] compressedBytes, bool parseNow)
        {
        ...
        public void SetImage(Bitmap png, int format)
        {
            this.png = png;
            this.Format = format;
            CompressPng(png);
                ...
                case 1:
                    {
                        ...
                        for (int i = 0; i < height; i++)
                            for (int j = 0; j < width; j++)
                            {
                                ...
                                buf[curPos] = (byte)(curPixel.B & 0xF);
                                buf[curPos] |= (byte)(curPixel.G << 4);
                                buf[curPos + 1] = (byte)(curPixel.R & 0xF);
                                buf[curPos + 1] |= (byte)(curPixel.A << 4);
                                curPos += 2;
                            }
